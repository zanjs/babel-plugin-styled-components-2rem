...
const getName = (node, t) => {
  ...
  throw path.buildCodeFrameError(
    `Cannot infer name from node with type "${node.type}". Please submit an issue at github.com/styled-components-2rem/babel-plugin-styled-components-2rem with your code so we can take a look at your use case!`
  )
}
...
export default t => (path, state) => {
  ...
  // state.customImportName is passed through from styled-components-2rem/macro if it's used
  ...
  if (!importName || !bindings[importName.name] || !bindings[importName]) {
    addDefault(path, 'styled-components-2rem', {
      nameHint: 'styled',
    })
